...
def computeBasis(ns):
    ...
    L[:, 2] = ns[:, 0]
    L[:, 3] = ns[:, 2]
    L[:, 4] = np.multiply(ns[:, 0], ns[:, 1])
    L[:, 5] = np.multiply(ns[:, 1], ns[:, 2])
    L[:, 6] = 3 * np.power(ns[:, 2], 2) - 1
    L[:, 7] = np.multiply(ns[:, 0], ns[:, 2])
    L[:, 8] = np.power(ns[:, 0], 2) - np.power(ns[:, 1], 2)
...
if __name__ == '__main__':
    ...
    ##################################################
    # OLS
    coeff = np.dot(np.dot(np.linalg.inv(np.dot(B.T, B)), B.T), vs)
    ...
    # TLS
    A = np.column_stack((B, vs))
    eigenvalues, eigenvectors = np.linalg.eig(np.dot(A.T, A))
    alpha = min(eigenvalues)
    coeff = np.dot(np.dot(np.linalg.inv(np.dot(B.T, B) - alpha*np.identity(9)), B.T), vs)
    img = relightSphere(tennis,coeff)
    output = compositeImages(img,target)
    print('Coefficients:\n'+str(coeff))
    plt.figure(2)
    ...
    # Scaled TLS
    coeff = np.dot(np.dot(np.linalg.inv(np.dot(B.T, B) - (alpha*384/255)*np.identity(9)), B.T), vs)
    img = relightSphere(tennis,coeff)
    output = compositeImages(img,target)
    print('Coefficients:\n'+str(coeff))
    plt.figure(3)
